...
const Slide = ({ slide, index, current, handleSlideClick }) => {
  ...
  return (
    <div className="[perspective:1200px] [transform-style:preserve-3d]">
      <li
        ref={slideRef}
        className="flex flex-1 flex-col items-center justify-center relative text-center text-white w-[120vmin] h-[80vmin] mx-[4vmin] z-10"
      ...
      >
        <div
          className="absolute top-0 left-0 w-full h-full bg-[#1D1F2F] rounded-[1%] overflow-hidden flex items-center justify-center transition-all duration-150 ease-out"
          style={{
            transform:
              current === index
        ...
        >
          <h2 className="text-lg md:text-2xl lg:text-4xl ">{title}</h2>
        </article>
      </li>
    </div>
  );
...
function Carousel({ slides }) {
  ...
  return (
    <div
      className="relative w-[120vmin] h-[80vmin] mx-auto"
